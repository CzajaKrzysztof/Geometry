...
`java -jar lib/junit-platform-console-standalone-1.4.0.jar -cp out/test/:out/production/ --scan-classpath`
...
`find src/test -name "*.java" > test.txt && mkdir -p out/test && javac -cp lib/\*:out/production:. @test.txt -d out/test && java -jar lib/junit-platform-console-standalone-1.4.0.jar -cp out/test/:out/production/ --scan-classpath`
